...
function saveDailyNotes() {
  const activeDate = appState.currentDate;
  const notesText = document.getElementById('settings-day-notes').value.trim();
  
  if (appState.dailyRecords[activeDate]) {
    appState.dailyRecords[activeDate].notes = notesText;
    saveState();
    showToast('Daily ledger notes saved successfully.');
  }
}
...
function openMorningOpeningModal() {
  ...
  ACCOUNTS.forEach(acc => {
    const val = dayData.openingBalances[acc] || 0;
    
    const fieldBox = document.createElement('div');
    fieldBox.className = 'opening-input-box';
    fieldBox.innerHTML = `
      <label for="op-${acc.replace(/\s+/g, '')}">${acc}</label>
      <input type="number" id="op-${acc.replace(/\s+/g, '')}" data-bank="${acc}" value="${val}" step="any" required>
    `;
